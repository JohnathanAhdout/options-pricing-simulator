...
import numpy as np  # every random draw and every vectorized path computation in this module


def gbm_terminal(S0: float, T: float, r: float, sigma: float, q: float, n_paths: int, rng: np.random.Generator) -> np.ndarray:  # returns an array of n_paths terminal prices, not a single scalar
...
def gbm_path(S0: float, T: float, r: float, sigma: float, q: float, n_steps: int, n_paths: int, rng: np.random.Generator) -> np.ndarray:  # returns a 2D array: one row per path, one column per time step
...
def regime_switching_gbm_path(
    ...
    dt: float,
    r: float,
    vols: np.ndarray,  # one volatility per hidden regime, e.g. [0.12, 0.35] for calm/turbulent
    transition_matrix: np.ndarray,  # transition_matrix[i, j] = P(jump to regime j | currently in regime i)
    rng: np.random.Generator,
    initial_state: int = 0,
) -> tuple[np.ndarray, np.ndarray]:  # returns (prices, states), NOT just prices: states is the ground truth the HMM never sees
    ...
    """
    n_states = len(vols)  # how many regimes exist, inferred from the length of the vols array rather than passed separately
    states = np.zeros(n_steps + 1, dtype=int)  # one regime label per time step, filled in below; dtype=int since these are category indices, not floats
    states[0] = initial_state  # the starting regime is given, not drawn, since there's no "previous" step to transition from
    log_returns = np.zeros(n_steps)  # preallocated; filled in one entry per iteration of the loop below
    ...
    for t in range(1, n_steps + 1):
        prev_state = states[t - 1]  # yesterday's regime, which the transition matrix conditions on
        ...
        Z = rng.standard_normal()  # this step's single random shock (not a batch, since this loop runs one t at a time)
    ...
    return prices, states  # both returned together: states is what a real trader would never get to observe directly
