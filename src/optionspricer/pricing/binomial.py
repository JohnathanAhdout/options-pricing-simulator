...
max(+/-(S * u^j * d^(N-j) - K), 0), which -- for a *fixed* tree, i.e. fixed
u, d, N -- is a piecewise **linear** function of the root spot S: bumping S
by a cent rescales every terminal node linearly and essentially never
flips which nodes are in- vs out-of-the-money, so the price is locally
exactly linear in S almost everywhere. Central-difference delta is fine
under this (it's measuring the slope of a line), but gamma -- the
*curvature* -- comes back as floating-point noise around zero, because
there genuinely is no curvature except at the measure-zero set of spot
prices where a terminal node crosses the strike. This module sidesteps
the problem entirely by reading delta, gamma, and theta directly off the
tree's own step-1 and step-2 node values (a standard technique, e.g. Hull's
*Options, Futures, and Other Derivatives*), which costs nothing extra --
those nodes are already computed during backward induction -- and isn't
fooled by the tree's piecewise-linear geometry.
"""
