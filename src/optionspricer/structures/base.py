...
only which legs are in it. That's deliberate: it means `payoff_at_expiry`,
`mark_to_market`, and `portfolio_greeks` below are written once, generically
over legs, instead of once per named strategy the way the payoff functions
usually get written.
